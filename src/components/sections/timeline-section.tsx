const LearningStep = ({ number, title, description }: { number: string, title: string, description: string }) => (
    <div className="group relative flex gap-6 pb-12 last:pb-0">
        <div className="absolute left-6 top-12 h-full w-0.5 bg-linear-to-b from-secondary/50 to-transparent group-last:hidden" />

        <div className="relative flex h-12 w-12 shrink-0 items-center justify-center rounded-full border border-secondary bg-foreground/10 backdrop-blur-xl text-xl font-bold text-secondary shadow-[0_0_20px_rgba(153,27,27,0.3)] transition-all group-hover:scale-110 group-hover:border-primary group-hover:shadow-[0_0_25px_rgba(234,179,8,0.2)]">
            {number}
        </div>

        <div className="flex flex-col pt-2 transition-transform group-hover:translate-x-1">
            <h3 className="text-xl font-semibold text-foreground group-hover:text-secondary transition-colors">{title}</h3>
        ...
        <section id="timeline-section" className="py-24 px-6 flex flex-col w-full max-w-4xl mx-auto">
            <div className="text-center mb-16">
                <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-4">
                    Alur Belajar di <span className="bg-linear-to-r from-primary via-secondary to-primary bg-clip-text text-transparent">Lampion</span>
                </h2>
